...
impl State {
    ...
    fn update(&mut self, td: std::time::Duration , )  {
        let dt = ANIMATION_SPEED * td.as_secs_f32();
...
fn main() {
    ...
    let strt_time = std::time::Instant::now(); // change later for some crate, to run in the browser 
    event_loop.run(move |event, _, control_flow| {
        match event {
            ...
            Event::RedrawRequested(_) => {
                let now =  std::time::Instant::now() ; // same thing here, change for a 3rd part crate
